...
public class ParamValidator
{
    ...
    {
        if (!(options.Protocol.ToLower()=="tcp"||options.Protocol.ToLower()=="udp"))
        {
            return false;
        }
        
        // -1 - Port didn't chosen
        if (options.DestinationPort > 65535 || options.DestinationPort < -1)
        {
            return false;
        }
        
        if (options.SourcePort > 65535 || options.SourcePort < -1)
        {
            return false;
        }
